...
%
% function A = sfo_max_dca_lazy(F,V,alpha)
% F: submodular function
% V: index set
% alpha (optional): specified accuracy level
% k (optional): constraint on #elements; requires F to be monotonic!
%
% returns solution A' s.t. F(A') >= max_A F(A) - alpha
%
% Example: A = sfo_max_dca_lazy(@sfo_fn_example,1:2)

function A = sfo_max_dca_lazy(F,V,alpha,k)
if ~exist('alpha','var')
    alpha = 0;
end
...
if exist('k','var')
    ...
    FT = @(A) F(A) - optbound*max(0,length(A)-k);
